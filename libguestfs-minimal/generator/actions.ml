...
You can use either C<guestfs_tune2fs_ll> or C<guestfs_get_e2label>
    ...
    tests = [
      InitNone, Always, TestResult (
        [["part_disk"; "/dev/sda"; "mbr"];
         ...
         ["ntfs3g_probe"; "true"; "/dev/sda1"]], "ret == 0"), [];
      InitNone, Always, TestResult (
        [["part_disk"; "/dev/sda"; "mbr"];
         ["mkfs"; "ext2"; "/dev/sda1"; ""; "NOARG"; ""; ""];
         ["ntfs3g_probe"; "true"; "/dev/sda1"]], "ret == 12"), []
    ];
...
C<wc -w> external command." };
  ...
  { defaults with
    ...
    tests = [
      InitISOFS, Always, TestResult (
        [["wc_cc"; "/100kallspaces"]], "ret == 102400"), []
    ];
...
C<guestfs_tune2fs_ll> and read the C<Block size> and C<Block count>
...
C<guestfs_tune2fs_ll>.  For precise details of how tune2fs
...
See also C<guestfs_journal_get_data_threshold>." };
  ...
  { defaults with
    name = "aug_setm";
    style = RInt "nodes", [String "bbase"; OptString "sub"; String "val"], [];
    proc_nr = Some 411;
    tests = [
      InitBasicFS, Always, TestResultString (
        [["mkdir"; "/etc"];
